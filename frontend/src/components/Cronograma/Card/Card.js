...
function Card(props) {
	...
	return (
		<>
			{mostrarModal && (
				<CardInfo onClose={() => trocarMostrarModal(false)} card={props.card} boardId={props.boardId} updateCard={props.updateCard}/>
			)}
			<div className="card" draggable onClick={() => trocarMostrarModal(true)}>
				<div className="card_top">
					...
					<div className="card_top_more" onClick={(event) => {
						...
						{showDropdown && (
							<Dropdown class="board_dropdown" onClose={() => setShowDropdown(false)}>
								<p onClick={() => props.removeCard(props.boardId, id)}>Deletar atividade</p>
							</Dropdown>
						)}
					</div>
				</div>
